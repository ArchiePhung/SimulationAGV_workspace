...
class ros_control():
	def __init__(self):
		...
		self.pub_vel = rospy.Publisher("/agv1/cmd_vel", Twist, queue_size=10)
		self.velPs2 = Twist()
		...
		self.address = "192.168.1.92" # "172.21.15.224"
	...
	def run_maunal(self):
		cmd_vel = Twist()
		val_linear = (self.app_button.vs_speed/100.)*0.7      #0.35
		val_rotate = (self.app_button.vs_speed/100.)*0.6      #0.3
	...
	def run(self):
		if self.process == -1: # khi moi khoi dong len
			...
			# self.listError =  self.synthetic_error()   # bỏ qua check lỗi  <<<<<<
