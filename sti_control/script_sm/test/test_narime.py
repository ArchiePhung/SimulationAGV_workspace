...
class TestCYNarime():
	def __init__(self):
		...
		self.TIME_CHECKPOS = 4.0
		self.TIME_LOG = 1
		self.TIME_CHECKFLOOR2 = 60.0
		self.TIME_CHECKDONE = 30.0
	...
	def confirm_pos(self):                       ## Setup time check xác nhận vị trí
		if self.step_checkpos == 0:
			...
			if rospy.get_time() - self.ct_checkpos < self.TIME_CHECKPOS:
				...
				if self.signal_CYMToyo.bit5_cnt == 1:    # Conveyor xác nhận vị trí với AGV + phần băng tải 1 đã ở vị trí sẵn sàng
	...
	def get_cy_number(self):
		...
		if self.mission == self.MISSION_TRSM:
			...
			if self.signal_conveyor11.sensor_limitAhead == 0 and self.signal_conveyor11.sensor_limitBehind == 0 and self.signal_conveyor12.sensor_limitAhead == 1 and self.signal_conveyor12.sensor_limitBehind == 1 and \
				self.signal_conveyor21.sensor_limitAhead == 1 and self.signal_conveyor21.sensor_limitBehind == 1 and self.signal_conveyor22.sensor_limitAhead == 1 and self.signal_conveyor22.sensor_limitBehind == 1:
				self.mission_val = 0b0001
			elif self.signal_conveyor11.sensor_limitAhead == 1 and self.signal_conveyor11.sensor_limitBehind == 1 and self.signal_conveyor12.sensor_limitAhead == 0 and self.signal_conveyor12.sensor_limitBehind == 0 and \
				self.signal_conveyor21.sensor_limitAhead == 1 and self.signal_conveyor21.sensor_limitBehind == 1 and self.signal_conveyor22.sensor_limitAhead == 1 and self.signal_conveyor22.sensor_limitBehind == 1:
				self.mission_val = 0b0010
			elif self.signal_conveyor11.sensor_limitAhead == 1 and self.signal_conveyor11.sensor_limitBehind == 1 and self.signal_conveyor12.sensor_limitAhead == 1 and self.signal_conveyor12.sensor_limitBehind == 1 and \
				self.signal_conveyor21.sensor_limitAhead == 0 and self.signal_conveyor21.sensor_limitBehind == 0 and self.signal_conveyor22.sensor_limitAhead == 1 and self.signal_conveyor22.sensor_limitBehind == 1:
				self.mission_val = 0b0100	
			elif self.signal_conveyor11.sensor_limitAhead == 1 and self.signal_conveyor11.sensor_limitBehind == 1 and self.signal_conveyor12.sensor_limitAhead == 1 and self.signal_conveyor12.sensor_limitBehind == 1 and \
				self.signal_conveyor21.sensor_limitAhead == 1 and self.signal_conveyor21.sensor_limitBehind == 1 and self.signal_conveyor22.sensor_limitAhead == 0 and self.signal_conveyor22.sensor_limitBehind == 0:
				self.mission_val = 0b1000
			elif self.signal_conveyor11.sensor_limitAhead == 0 and self.signal_conveyor11.sensor_limitBehind == 0 and self.signal_conveyor12.sensor_limitAhead == 0 and self.signal_conveyor12.sensor_limitBehind == 0 and \
				self.signal_conveyor21.sensor_limitAhead == 1 and self.signal_conveyor21.sensor_limitBehind == 1 and self.signal_conveyor22.sensor_limitAhead == 1 and self.signal_conveyor22.sensor_limitBehind == 1:
				self.mission_val = 0b0011
			elif self.signal_conveyor11.sensor_limitAhead == 0 and self.signal_conveyor11.sensor_limitBehind == 0 and self.signal_conveyor12.sensor_limitAhead == 1 and self.signal_conveyor12.sensor_limitBehind == 1 and \
				self.signal_conveyor21.sensor_limitAhead == 0 and self.signal_conveyor21.sensor_limitBehind == 0 and self.signal_conveyor22.sensor_limitAhead == 1 and self.signal_conveyor22.sensor_limitBehind == 1:
				self.mission_val = 0b0101	
			elif self.signal_conveyor11.sensor_limitAhead == 0 and self.signal_conveyor11.sensor_limitBehind == 0 and self.signal_conveyor12.sensor_limitAhead == 1 and self.signal_conveyor12.sensor_limitBehind == 1 and \
				self.signal_conveyor21.sensor_limitAhead == 1 and self.signal_conveyor21.sensor_limitBehind == 1 and self.signal_conveyor22.sensor_limitAhead == 0 and self.signal_conveyor22.sensor_limitBehind == 0:
				...
				self.signal_conveyor21.sensor_limitAhead == 0 and self.signal_conveyor21.sensor_limitBehind == 0 and self.signal_conveyor22.sensor_limitAhead == 0 and self.signal_conveyor22.sensor_limitBehind == 0:
				self.mission_val = 0b1100
			elif self.signal_conveyor11.sensor_limitAhead == 0 and self.signal_conveyor11.sensor_limitBehind == 0 and self.signal_conveyor12.sensor_limitAhead == 0 and self.signal_conveyor12.sensor_limitBehind == 0 and \
				self.signal_conveyor21.sensor_limitAhead == 0 and self.signal_conveyor21.sensor_limitBehind == 0 and self.signal_conveyor22.sensor_limitAhead == 1 and self.signal_conveyor22.sensor_limitBehind == 1:
				self.mission_val = 0b0111
	...
	def run_auto(self):
		if self.enb_conduct == 1 and self.mission == self.MISSION_TRSM:
			if self.case_step2 == 0:
				...
				if self.step_checkdone == 0:
					...
					if rospy.get_time() - self.ct_checkdone < self.TIME_CHECKDONE:
						...
						if self.signal_CYMToyo.bit6_done == 1:
		...
		elif self.enb_conduct == 1 and self.mission == self.MISSION_RECV:
	...
	def test_CYseparate(self, b1, b2, b3, b4, mss1, mss2, mss3, mss4, mssdone1, mssdone2, mssdone3, mssdone4):
		if self.enb_conduct == 1:
			if self.step == 0:
				if b3 == 0 and b4 == 0:
					self.AGVToyo_signal.bit1 = b1
					self.AGVToyo_signal.bit2 = b2
					self.step = 1
					print("AGV đã sẵn sàng trả/nhận tại tầng 1")
				else:
					self.AGVToyo_signal.bit3 = b3
					self.AGVToyo_signal.bit4 = b4
					if self.signal_CYMToyo.bit7_rdy == 1:
						self.step = 1
