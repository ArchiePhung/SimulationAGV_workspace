...
class ros_control():
	def __init__(self):
		...
		self.numQt = len(self.fakePl.QT1)
		...
		self.pub_moveReq = rospy.Publisher("/request_move", LineRequestMove, queue_size = 1024)
	...
	def checkStatus_mapMatchLoc(self):
		if self.lidarLoc_status.map_match_status > 30: 
			self.saveTime_detectMapMathError = rospy.Time.now()
			...
			if self.step_tryTarget == 0:
				...
				# -- Send vel
					# if self.lift_status.status.data == 0 or self.lift_status.status.data >= 3:  # Đang thực hiện nhiệm vụ ở chế độ auto -> ko cho phép di chuyển.
						# -- Move
						if (self.app_button.ck_remote == 0):
	...
	# -- RUN AUTO:
			...
			if (self.app_button.bt_doNextPlan or self.HC_info.zone_sick_ahead != 0) and self.status_goalControl.complete_misson == 0:
				if self.numQt == 0:
					print("khong co quy trinh!")

				else:
					print("qt now", self.qtNow)
					self.cmd_target_x = self.fakePl.QT1[self.qtNow].target_x
					self.cmd_target_y = self.fakePl.QT1[self.qtNow].target_y
					self.cmd_listInfoPath = self.fakePl.QT1[self.qtNow].pathInfo
				
			if ( self.target_x != self.cmd_target_x ) or ( self.target_y != self.cmd_target_y):
				if (self.cmd_target_x < 500.) and (self.cmd_target_y < 500.):
					""" Không cho phép đổi lệnh khi đang thao tác:
						1, Đang đi vào/ra điểm thao tác.
						2, Đang đi vào/ra khỏi sạc.
					"""
					# a1 = 0
					# a2 = 0
					...
					self.log_mess("info", "Have new target but Not fit: X= ", self.cmd_target_x )
					self.log_mess("info", "Have new target but Not fit: y= ", self.cmd_target_y)
					self.process = 2
			...
			#             delta_distance = self.calculate_distance(self.robotPose_lidarLOC.pose.position, self.poseWait.position)
			...
			#             delta_distance = self.calculate_distance(self.robotPose_lidarLOC.pose.position, self.pose_parkingRuning.position)
			#             # -- phien ban 2: Xac dinh do lech giua goc cua AGV voi goc cua diem vao Tag
			#             delta_angle = self.calculate_angle(self.robotPose_lidarLOC.pose.orientation, self.parking_poseTarget.orientation)
			...
			#         delta_distance = self.calculate_distance(self.robotPose_lidarLOC.pose.position, self.cancelbackward_pose.position)
			#         delta_angle = self.calculate_angle(self.robotPose_lidarLOC.pose.orientation, self.cancelbackward_pose.orientation)
		...
		# -- Parking	
			...
			if self.completed_moveSpecial == 0: # chua hoan thanh di chuyen
				...
				if self.parking_status.status == 1: # Free
					...
					# -- add 12/11/2021
					self.poseWait = self.robotPose_lidarLOC.pose

					# -- add 27/12/2021
					self.cancelbackward_pose = self.robotPose_lidarLOC.pose
					self.cancelbackward_offset = self.parking_offset
				else:
					self.process = 2

				# -- add 23/12/2021:
				self.pose_parkingRuning = self.robotPose_lidarLOC.pose
			else:
