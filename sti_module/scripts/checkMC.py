    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
                    ...
                    if self.end_of_list == True:
                        ...
                        if self.is_target_change == True:
                            ...
                            if self.id_fl != 0.0:
                                ...
                                elif self.fnCalcDistPoints(self.poseRbMa.position.x, self.cur_goal_x, self.poseRbMa.position.y, self.cur_goal_y) > self.ss_luivsnextGoal:
                                    self.is_need_turn_step1 = 1
                                    self.is_need_pttt = 1
                                    self.point_goal_start_x = self.poseRbMa.position.x
                                    self.point_goal_start_y = self.poseRbMa.position.y
            ...
            #----- dieu huong AGV
                ...
                self.vel_x_control = 0.4
                # new traffic
                # if self.vel_fl == 0: # roi vao th khong xac dinh
                #     self.vel_x_control = 0.3
                    
                # else:
                #     self.vel_x_control = (self.vel_fl/255)*self.vel_x_max
