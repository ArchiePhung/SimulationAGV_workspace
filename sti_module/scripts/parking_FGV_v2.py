...
class ParkingNAV(threading.Thread):
    ...
    def gotoWaitPoint(self, dis_tolerance, ang_tolerance, velocity_max, vel_rotMax):
        twist = Twist()
        ...
        if poseThenTransform.position.x == 0.0 and poseThenTransform.position.y == 0.0 and poseThenTransform.orientation.z == 0.0 and poseThenTransform.orientation.w == 0.0 :
            ...
            if distance >= permission_tolerance and fabs(angleAGV) >  ang_tolerance:
                kpd1 = 0.5
                vel_x = kpd1*distance
    ...
    def run(self):
        ...
        while not self.shutdown_flag.is_set():
            ...
            elif self.process == 41: # di chuyen den diem cho
                stt = self.gotoWaitPoint(0.01, 0.015, 0.3, 0.7)
                if stt == 1:
                    time.sleep(10.)
                    self.process == 42
                elif stt == 2:
            ...
            elif self.process == 42: # parking vao ke
                # print("Start Parking!")
