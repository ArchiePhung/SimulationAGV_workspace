...
class goalControl():
    def __init__(self):
        ...
        self.distance_goArc = rospy.get_param('~distance_goArc', 1.5)
    ...
    def pub_status(self, misson, status_now, error, safety, complete_misson , id):
        ...
        if misson == 1 and complete_misson == 0 and self.end_of_list == True:
    ...
    def find_point_special(self, x_c_goal, y_c_goal, x_n_goal, y_n_goal, dk_check):
        ...
        if dk_check == 1:
            if goc >= 88.0*PI/180.0 and goc < 160.0*PI/180.0 and d2 > self.distance_goArc:
    ...
    def update_all(self):
        ...
        if len(self.req_move.list_x) != 0 and len(self.req_move.list_y) != 0 and self.req_move.list_id[0] != 0.0:
            ...
            if len(self.req_move.list_speed) != 0:
    ...
    def save_oldlist(self):
        self.old_list_x = self.req_move.list_x
        self.old_list_y = self.req_move.list_y
        self.old_target_x = self.req_move.target_x
        self.old_target_y = self.req_move.target_y
        
    def free_oldlist(self):
        self.old_list_x = self.list_unknow
        self.old_list_y = self.list_unknow
        self.old_target_x = self.coordinate_unknown
        self.old_target_y = self.coordinate_unknown
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # tim ra current goal, next goal
                ...
                # update list x,y
                if self.old_target_x == self.req_move.target_x and self.old_target_y == self.req_move.target_y and self.old_list_x == self.req_move.list_x and self.old_list_y == self.req_move.list_y:
            ...
            #----- dieu huong AGV
                ...
                if self.cur_goal_is == 2 and (self.distance_goal <= self.tol_simple or self.kc_con_lai <= self.tol_simple or self.is_over_goal == True) :
                    ...
                    self.stt_agv = 1
                    # if fabs(self.vel_raw.twist.twist.linear.x) >= 0.4:
                    #     self.selectfield(0) # chon vung rong
                    # else:
                    #     self.selectfield(1) # chon vung nho
