...
class goalControl():
    ...
    def find_point_special(self, x_c_goal, y_c_goal, x_n_goal, y_n_goal):
        ...
        if goc >= 160.0*PI/180.0 or (goc >= 88.0*PI/180.0 and d1 > 0.8 and d2 > 1.0):
            rospy.loginfo('x = %lf, y = %lf khong la diem dac biet',x_c_goal,y_c_goal)
            return False
        else:
            rospy.loginfo('x = %lf, y = %lf la diem dac biet',x_c_goal,y_c_goal)
                ...
                if self.stt_agv == 0 : # AGV tinh goal tiep theo sau khi dung
                    ...
                    if self.end_of_list == True:
                        ...
                        if self.is_target_change == True:
                            ...
                            if self.id_fl != 0.0:
                                if self.fnCalcDistPoints(self.poseRbMa.position.x, self.cur_goal_x, self.poseRbMa.position.y, self.cur_goal_y) > self.ss_luivsnextGoal or ((round(self.cur_goal_x, 3) == round(self.target_x, 3)) and (round(self.cur_goal_y, 3) == round(self.target_y, 3))):
