...
#include <sti_msgs/Status_getPoseAruco.h>
#include <sti_msgs/webConsole_receive.h>
#include <string>
#include <iostream>
#include <fstream>
#include <stdio.h>
...
std_msgs::Int16 key ;
sti_msgs::Status_getPoseAruco data_stt;
string file = "/home/amr1-l300/catkin_ws/src/sti_module/data/data_tag.json" ;
ros::Publisher  pub_status;

bool sub4 = false, sub5 = false, sub6 = false ;
    ...
    void callSub6(const sti_msgs::Status_getPoseAruco& stt_spa){
        data_stt = stt_spa ;
        sub6 = true ;
    }

    void pub(int id_tag, int sl_tag, int is_complete){
        sti_msgs::webConsole_receive stt ;
        stt.Id_tag = id_tag ;
        stt.sl_tag = sl_tag ;
        stt.complete = is_complete ;

        pub_status.publish(stt);
    }
    ...
    void writeData(geometry_msgs::PoseStamped& tag_data, string name_file)
    {
        Json::Reader reader;
        Json::Value val;
        ifstream ifs; // khai bao bien doc file
        ifs.open(name_file);
        reader.parse(ifs, val);
        string tag_name = *(&tag_data.header.frame_id);
        val[tag_name]["x"] = tag_data.pose.position.x;
        val[tag_name]["y"] = tag_data.pose.position.y;
        val[tag_name]["z"] = tag_data.pose.position.z;
        val[tag_name]["rx"] = tag_data.pose.orientation.x;
        val[tag_name]["ry"] = tag_data.pose.orientation.y;
        val[tag_name]["rz"] = tag_data.pose.orientation.z;
        val[tag_name]["rw"] = tag_data.pose.orientation.w;
        ...
        ofstream outputFileStream(name_file);
        writer -> write(val, &outputFileStream);
        ifs.close();
...
int main(int argc, char** argv){
    ...
    // Publisher
        pub_status    = n.advertise<sti_msgs::webConsole_receive>("setPoseAruco_status", 100);
    ...
    // subcriber
        ...
        ros::Subscriber subscribe6 = n.subscribe("/setPoseAruco_control", 10, callSub6);
    ...
    while(n.ok()){
        ...
        if(sub4 == true){
            ...
            if ( sl_tag != 0 ){
                ...
                if(((sub5 ==true && key.data == 13) || (sub6 ==true && data_stt.status_confirnm != 0)) && sl_tag ==1){ // enter
                    sub5 = false ;
                    sub6 = false;
