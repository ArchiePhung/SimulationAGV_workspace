    ...
    def run(self):
        ...
        while not rospy.is_shutdown():    
            if self.process == 0:
                ...
                if self.req_parking.modeRun == 0: # wait reset!
                    self.resetAll()
                    print("Reset parking!")
                    self.process = 1
            ...
            self.pub_Status(self.process, self.req_parking.modeRun, self.req_parking.poseTarget, self.req_parking.offset, self.poseParking.position.x, self.poseParking.position.y, self.theta_robotParking, "mess", 0)   
            self.rate.sleep()
