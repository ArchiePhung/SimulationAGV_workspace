...
class Program(threading.Thread):
	def __init__(self, threadID):
		...
		self.pathDataPose = '/home/tiger/simulation_ws/src/app_ros/config/dataSetPose.json'
		# --
		...
		rospy.Subscriber("/agv_pose", Pose, self.callback_statusLOC)
		# self.status_loc = Pose()
		# -- Info Move
	...
	def loadDataSetPose(self):
		...
		try:
			...
			if self.listIdSetpose != listId:
				self.listIdSetpose = listId
				self.dataDicSetPose = json_object

				self.valueLable.isUpdateListIdSetpose = True
				self.valueLable.listIDSetpose = self.listIdSetpose
		except:
			pass
	...
	def controlSetPose(self):
		...
		call = self.callServiceInitializeAtPose(dX, dY, dG, fradius)
		if call:
			self.statusColor.lbc_setpose = 1
		else:
			self.statusColor.lbc_setpose = -1
	...
	def controlAll(self):
		...
		if self.status_loc.map_match_status == 90:
			self.valueLable.lbv_map_match_status = 'Good'
		elif self.status_loc.map_match_status == 60:
			self.valueLable.lbv_map_match_status = 'Medium'
		elif self.status_loc.map_match_status == 30:
			self.valueLable.lbv_map_match_status = 'Low'
		else:
			self.valueLable.lbv_map_match_status = 'None'
		...
		if self.status_loc.loc_status == 10:
			...
			self.valueLable.lbv_loc_status = 'Not Loc'
		else:
			self.valueLable.lbv_loc_status = 'Sys Err'
